...
\begin{equation}
2 k i \frac{\partial \tilde{U}}{\partial z} - 4 \pi^2 (u^2 + v^2) = 0.
\end{equation}
\inred{The solution must verify,}
\begin{equation}\label{eq: propagator reciprocal space}
\tilde{U}(u,v,z) = \tilde{U}(u,v,0)  e^{- \pi i \lambda z (u^2 + v^2) }
\end{equation}
what means that in the reciprocal space, $\tilde{U}(u,v,z)$ that is the propagated wavefield at a distance $z$ is just the unpropagated field multiplied by a 2D Gaussian $\tilde{G}(u,v) =  \exp(- \pi i \lambda z (u^2 + v^2) )$. This Gaussian in reciprocal space is itself originated from another a Gaussian in real space
\begin{equation}
...
\begin{equation}
U(x,y,z) = \int\int e^{i 2 \pi (u x + v y) } \tilde{U}(u,v,0)  \tilde{G}(u,v)  du dv.
\end{equation}
...
\end{equation}
or expression of the Fresnel propagator. An equivalent form of the Fresnel propagator is obtained expanding the exponent in the ingtegrand, then factorizing and taking out of the integral the term that does not depend on the variables of integration
...
The case when $e^{\frac{i k}{2 z} (x^{\prime 2} + y^{\prime 2} )} \simeq 1$ is called the Fraunhofer approximation. From Eq.~\ref{eq: Fresnel far field} one obtains 
\begin{equation}\label{eq: fraunhoffer}
U(x,y,z) = \frac{\inred{e^{i k z}}}{i \lambda z}
\int\int U(x',y',0) e^{ \frac{i k}{2 z} (x'^2 + y'^2) } 
e^{ \frac{-i 2 \pi}{z \lambda} (x x' + y y') } dx' dy',
\end{equation}
It is valid when the propagation distance verify $z >>k (x^{\prime 2}_{max} + y^{\prime 2}_{max})/2 $.  

\subsection{Beyond the Fresnel approximation}

In the rigorous theory of diffraction the scalar Helmholtz equation is solved for general boundary conditions. Its development is not trivial and need particular mathematical tools, like the Green's theorem, with clever use of the boundary conditions. Refs.~\cite{bornwolf}, \cite{nieto} and other texts in Optics present this theory. The gerenal procedure is: i) write the solution of Helmholz Equation in the form of the integral theorem of Helmholz and Kirchhoff (pag 377, Eq.~7 in \cite{bornwolf}), ii) consider the Kirchhoff approximation, that assumes that the boundary values of the solution at the boundaries can be determined by geometrical optics to obtain the Fresnel-Kirchhoff diffraction formula (pag. 380, Eq.~17)
, and iii) then  deduce the Fresnel diffraction formula
\end{itemize}
% 
