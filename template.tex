...
\end{equation}
where $r'$ is a normalized radial coordinate $r' = \sqrt{(x^2 + y^2)/(\lambda \Delta z)}$, and $J_0$ the first-kind Bessel function of zeroth order. This integral has no analytical form but can be easily integrated numerically. Fig.~\ref{fig: 2D aperture comparison} shows a zoom of the central part of the profile at $y=0$ in Fig.~\ref{fig: SRW aperture}a calculated with different libraries. Note that the abscissas grid is not dense enough to follow in detail the oscillations. If one wants to increase the spatial resolution by a factor of 2, one has to extend the window ar the source by a factor of 2 in both directions. To keep the same pixel size at the source, one must consequently increase also by a factor of 2 the number of points. The result is in Fig.~\ref{fig: SRW aperture}b. 
\begin{figure}
\label{fig: SRW aperture}
