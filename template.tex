...
\begin{abstract}
For many applications in synchrotron radiation, both for instrumentation and data analysis, the x-rays wavefronts have to be propagated from one spatial point to another. We describe here the integral propagators based on the Fresnel diffraction equation that are used to propagate the x-ray wavefront in free space.  We describe the variety of numerical implementations used in several simulation packages and libraries, discussing the cases where they can be applied with particular attention to the sampling conditions. 
\end{abstract}
...
\todo{RAPHAEL to write this section, OLEG to review it. Use \cite{ChubarCelestre}}
...
This section presents the formalism used for a propagator that makes possible to calculate the propagated field in a ``zoomed'' window, thus permitting optimizing the wavefront sampling in cases for propagating highly convergent or divergent wavefronts. We follow the derivation in J.D. Schmidt \cite{schmidt} extended for including different zoom factors in horizontal and vertical. 
...
%\inred{Interestingly this picture changed after upgrading my Mac OS... In particular SRW result. Now the agreement is much better!! Why??}
}
...
\subsection{The semianalytical propagator}
\todo{RAFAEL, please...}
