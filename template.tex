...
% 
% To proceed further into obtaining the free-space wave equation for the electric field the following relations comes in handy:
...
With some manipulations one can arrive to the Wave equation
...
\end{equation}
which is the d'Alembert wave equation for electric field. 
The same equation holds for the magnetic induction (replacing $\mathcal{E} \longleftrightarrow \mathcal{B}$). 
% Applying the same approach yields the vacuum field equation for the magnetic induction:
...
Solving the scalar Helmholtz equation for general boundary conditions is not trivial and need sophisticated mathematical tools, like the Green's theorem, with clever use of the boundary conditions. However, we follow here another more intuitive approach called the angular spectrum method.  Exact and approximate methods for solving diffraction problems based on the Helmholtz equation will be shown. It has the advantage of being more intuitive and free from some of the subtle difficulties of boundary conditions encountered when solving the Helmholtz equation exploiting the Green's theorem while (cf. Goodman \cite{goodmanfourier}).
...
\end{equation}
There are three necessary steps: 1) Fourier transform the unpropagated field: $\mathcal{F}\big\{U(x,y,0)\big\}$, 2) Multiply it for the free-space Fresnel propagator $ K=\exp(-i \Delta z (k_x^2 + k_y^2) / (2k))$, 3) Inverse-Fourier transform this result with respect to $k_x$ and $k_y$:  $\mathcal{F}^{-1}\Big\{\mathcal{F}\big\{U \big\} K \Big\}$, and 4) apply a global phase $P^G = {e^{ik\Delta z}}/(2 \pi)$ to the result \todo{check $2 \pi$}.
...
\subsection{Example propagate to the focal position of a highly converging wavefront}

\todo{EXAMPLE: Plane wave clipped by a slit, focused by an ideal lens. and propagate to close to the focal position. Like in Fig.~\ref{fig: slit 2D converging}. Discuss the wavefront evolution. FIND AND COMPARE WITH THE ANALYTICAL SOLUTION WITH SPHERICAL WAVE? May be discuss the Gouy phase?}
...
\todo{discuss the case of a wavefront curved with a radius, like $A=B/R$ }

The condition in Eq.~\ref{eq: good sampling condition} should be applied to all quadratic exponential phases in all propagators. 
...
\end{equation}
For the Fresnel propagators we can use these expressions and set $m=1$ so they reduce to the unique condition
\begin{equation}\label{eq: good sampling fresnel}
\Delta z < \frac{N \delta_x^2}{\lambda}
\end{equation}
\todo{For example, under the conditions of Fig.~\ref{fig: aperture_1D} the condition in Eq.~\ref{eq: good sampling fresnel} is represented in Fig.~\ref{}.}
\begin{figure}
